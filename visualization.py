...
class CircleNotation(Visualization):
    ...
    def draw(self):
        ...
        self._fig = plt.figure(constrained_layout=True, dpi=300)
        ax = self._fig.gca()
        ...
        for i in range(2 ** self._sim._n):
            ...
            phase = mlines.Line2D([xpos, xpos + lx[i]], [ypos, ypos + ly[i]], color=self._colors['phase'],
                                  linewidth=self._widths['phase'])
            ax.add_artist(fill)
            ax.add_artist(ring)
            ax.add_artist(phase)
...
class DimensionalCircleNotation(Visualization):
    ...
    def _drawDottedLine(self, points: list):
        self._ax.plot(self._coords[points, 0], self._coords[points, 1], color=self._colors['cube'],
                      linewidth=self._widths['cube'], linestyle='dotted', zorder=1)

    def _drawLine(self, points: list):
        self._ax.plot(self._coords[points, 0], self._coords[points, 1], color=self._colors['cube'],
                      linewidth=self._widths['cube'], linestyle='solid', zorder=1)
    ...
    def _drawCircle(self, index: int):
        ...
        if self._sim._n == 3:
            ...
            off = -1.3
        self._ax.text(xpos, ypos + off, fr'$|{label:s}\rangle$', size=self._widths['textsize'], usetex=False,
                      horizontalalignment='center', verticalalignment='center')
